### 입력
- 다리 길이
  - [x] 다리의 길이를 문자로 입력 받는다.
  - [x] 유효성 검증을 실시한다.
    - [x] 빈칸은 아닌지 확인한다
    - [x] 공백은 없는지 확인한다
    - [x] 입력이 숫자인지 확인한다 (정규표현식 사용)
    - [x] 길이는 3이상 20 이하이다
    - [x] 올바른 값이 아니면 예외 처리한다
      - `IllegalArgumentException`을 사용한다
      - 에러 문구는 "[ERROR]"로 시작해야 한다.
    - [x] 올바르지 않는 입력을 받는 경우 다시 입력을 받는다

- [x] 플레이어가 이동할 칸
  - 라운드마다 이동할 칸을 입력 받는다
  - [x] 위 칸 이동 희망의 경우 대문자 U, 아래 칸의 경우 대문자 D를 받는다
  - [x] U와 D가 아닌 경우 예외처리한다
      - `IllegalArgumentException`을 사용한다
      - 에러 문구는 "[ERROR]"로 시작해야 한다.
  - [x] 올바르지 않는 입력을 받는 경우 다시 입력을 받는다  

- [x] 게임 재시작/종료 여부 입력을 받는다
  - [x] 재시작을 원할 경우 R, 종료하고 싶을 경우 Q
  - [x] 올바른 값이 아닐 경우 예외처리
    - `IllegalArgumentException`을 사용한다
    - 에러 문구는 "[ERROR]"로 시작해야 한다.
  - [x] 올바르지 않는 입력을 받는 경우 다시 입력을 받는다
